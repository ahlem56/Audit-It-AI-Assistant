...
export default function ReportPage() {
  ...
  const requestedReportMissionId = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (!activeMission || reportPreview?.structured_output || loadingReport) return;
    if (requestedReportMissionId.current === activeMission.mission_id) return;
    requestedReportMissionId.current = activeMission.mission_id;
    void loadReportPreview();
  }, [activeMission?.mission_id, reportPreview?.structured_output, loadingReport]);

  useEffect(() => {
    if (!activeMission || qualityGate || loadingQualityGate || !reportPreview?.structured_output) return;
    void loadQualityGate();
  }, [activeMission?.mission_id, reportPreview?.structured_output, qualityGate, loadingQualityGate]);
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-card">
          <p className="text-sm text-slate-500">
            {loadingReport
              ? 'The report preview is loading. This can take a moment while the report is generated.'
              : 'The report preview is still loading or no structured report draft is available for this mission yet.'}
          </p>
          {!loadingReport && (
            <button
              type="button"
              onClick={() => void loadReportPreview()}
              className="mt-5 rounded-2xl bg-red-600 px-4 py-3 text-sm font-semibold text-white hover:bg-red-700"
            >
              Load report preview
            </button>
          )}
  ...
  return (
    <>
      <div className="space-y-8">
        ...
        <div className="grid min-w-0 gap-6 xl:grid-cols-[220px_minmax(0,1fr)_320px]">
          ...
          <main className="min-w-0 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-card sm:p-8">{renderSection()}</main>

          <aside className="space-y-5">
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-card">
              <div className="mb-4">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Export report</p>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  {effectiveQualityGate?.blocking_issues_count
                    ? `${effectiveQualityGate.blocking_issues_count} quality issue(s) detected. Export remains available.`
                    : 'Quality checks passed. The report is ready to export.'}
                </p>
              </div>
              <div ref={exportMenuRef} className="relative">
                <button
                  ...
                  aria-haspopup="menu"
                  className="inline-flex w-full items-center justify-between gap-3 bg-[#202936] px-4 py-3 text-sm font-semibold text-white hover:bg-slate-900 disabled:opacity-50"
                >
                  <span className="inline-flex items-center gap-2">
                    <Download className="h-4 w-4" />
                    {downloading ? 'Exporting PowerPoint...' : downloadingPdf ? 'Exporting PDF...' : downloadingDocx ? 'Exporting Word...' : 'Export report'}
                  </span>
                  <ChevronDown className={`h-4 w-4 transition-transform ${exportMenuOpen ? 'rotate-180' : ''}`} />
                </button>

                {exportMenuOpen && (
                  <div role="menu" className="report-export-menu absolute right-0 z-30 mt-2 w-full border border-slate-200 bg-white p-1.5">
                    <button type="button" role="menuitem" onClick={() => { setExportMenuOpen(false); void handleOpenExportModal(); }} className="report-export-option flex w-full items-center gap-3 px-3 py-2.5 text-left">
                      <Download className="h-4 w-4 text-slate-500" />
                      <span><span className="block text-sm font-semibold text-slate-900">PowerPoint</span><span className="block text-xs text-slate-500">Editable presentation (.pptx)</span></span>
                    </button>
                    <button type="button" role="menuitem" onClick={() => { setExportMenuOpen(false); void handleExportPdf(); }} className="report-export-option flex w-full items-center gap-3 px-3 py-2.5 text-left">
                      <ShieldCheck className="h-4 w-4 text-slate-500" />
                      <span><span className="block text-sm font-semibold text-slate-900">PDF</span><span className="block text-xs text-slate-500">Fixed review copy (.pdf)</span></span>
                    </button>
                    <button type="button" role="menuitem" onClick={() => { setExportMenuOpen(false); void handleExportDocx(); }} className="report-export-option flex w-full items-center gap-3 px-3 py-2.5 text-left">
                      <FileText className="h-4 w-4 text-slate-500" />
                      <span><span className="block text-sm font-semibold text-slate-900">Word</span><span className="block text-xs text-slate-500">Editable document (.docx)</span></span>
                    </button>
                  </div>
                )}
              </div>
              {exportError && <p className="mt-3 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">{exportError}</p>}
            </div>
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-card">
              ...
              <div className="report-quality-issues mt-4 max-h-80 space-y-3 overflow-y-auto pr-2">
            ...
            <div className="flex flex-wrap items-center justify-end gap-3 border-t border-slate-200 px-6 py-5 sm:px-8">
              ...
              <button
                ...
                disabled={
                  downloading ||
                  sendingEmail ||
                  loadingEmailDefaults ||
                  (sendByEmail && (!emailForm.to_email.trim() || !emailForm.subject.trim() || !emailForm.body.trim()))
                }
      ...
      {showFeedbackModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-4 sm:p-6">
          <div className="w-full max-w-2xl rounded-[2rem] bg-white p-5 shadow-2xl sm:p-8">
